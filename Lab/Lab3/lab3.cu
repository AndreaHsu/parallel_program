...
int main(int argc, char** argv) {
    ...
    unsigned img_size = height * width * channels;
    unsigned char* host_t = (unsigned char*) malloc(img_size * sizeof(unsigned char));
    
    /* Hint 1 */
    // cudaMalloc(...) for device src and device dst
    unsigned char *device_s, *device_t;  
    cudaMalloc((void**) &device_s, img_size * sizeof(unsigned char));
    cudaMalloc((void**) &device_t, img_size * sizeof(unsigned char));
    ...
    int k = 16; // 32: WA??, 16: 24.55, 8: 24.86, 4: 26.56
    dim3 threadsPerBlock(k, k); // set the number of threads in a block in dim=x is k, so did dim=y
    dim3 numberOfBlocks(width/k, height/k);
    sobel<<<numberOfBlocks, threadsPerBlock>>>(device_s, device_t, height, width, channels);
    
    /* Hint 4 */
    // cudaMemcpy(...) copy result image to host
    cudaMemcpy(host_t, device_t, img_size * sizeof(unsigned char), cudaMemcpyDeviceToHost);
    cudaFree(device_s); 
    cudaFree(device_t);
