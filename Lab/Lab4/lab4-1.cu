...
__global__ void sobel(unsigned char *sour, unsigned char *t, unsigned height, unsigned width, unsigned channels) {
    ...
    float val[Z][3];
    if(x >= width) return;
    __shared__ unsigned char sR[5][265];
    __shared__ unsigned char sG[5][265];
    __shared__ unsigned char sB[5][265];
    ...
    for(int v = -yBound; v <= yBound; v++){
        if(bound_check(y + v, 0, height)){
            if(threadIdx.x == 0 && blockIdx.x != 0){
                for(int i = -2; i < 0; i++){
                    sR[yBound + v][i+2] = sour[channels * (width * (y + v) + x + i) + 2];
                    sG[yBound + v][i+2] = sour[channels * (width * (y + v) + x + i) + 1];
                    sB[yBound + v][i+2] = sour[channels * (width * (y + v) + x + i) + 0];
                }
            }
            sR[yBound + v][threadIdx.x+2] = sour[channels * (width * (y + v) + x) + 2];
            sG[yBound + v][threadIdx.x+2] = sour[channels * (width * (y + v) + x) + 1];
            sB[yBound + v][threadIdx.x+2] = sour[channels * (width * (y + v) + x) + 0];
            if(threadIdx.x == blockDim.x - 1){
                for(int i = 1; i < 3; i++){
                    if(bound_check(x+i, 0, width)){
                        sR[yBound + v][threadIdx.x+2+i] = sour[channels * (width * (y + v) + x + i) + 2];
                        sG[yBound + v][threadIdx.x+2+i] = sour[channels * (width * (y + v) + x + i) + 1];
                        sB[yBound + v][threadIdx.x+2+i] = sour[channels * (width * (y + v) + x + i) + 0];
                    }
                }
            }
        }
        ...
        for (int v = -yBound; v <= yBound; ++v) {
            ...
            for (int u = -xBound; u <= xBound; ++u) {
                if (bound_check(x + u, 0, width) && bound_check(y + v, 0, height)) {
                    const unsigned char R = sR[yBound + v][threadIdx.x + u + 2];
                    const unsigned char G = sG[yBound + v][threadIdx.x + u + 2];
                    const unsigned char B = sB[yBound + v][threadIdx.x + u + 2];
