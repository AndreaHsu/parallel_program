...
__global__ void sobel(unsigned char *sour, unsigned char *t, unsigned height, unsigned width, unsigned channels) {
    ...
    for(int v = -yBound; v <= yBound; v++){
        if(bound_check(y + v, 0, height)){
            if(threadIdx.x == 0 && blockIdx.x != 0){
                #pragma unroll 2
                for(int i = -2; i < 0; i++){
                    s[channels * (sw * (yBound + v) + 2 + i) + 2] = sour[channels * (width * (y + v) + x + i) + 2];
                    s[channels * (sw * (yBound + v) + 2 + i) + 1] = sour[channels * (width * (y + v) + x + i) + 1];
                    s[channels * (sw * (yBound + v) + 2 + i) + 0] = sour[channels * (width * (y + v) + x + i) + 0];
                }
            }
            s[channels * (sw * (yBound + v) + threadIdx.x + 2) + 2] = sour[channels * (width * (y + v) + x) + 2];
            s[channels * (sw * (yBound + v) + threadIdx.x + 2) + 1] = sour[channels * (width * (y + v) + x) + 1];
            s[channels * (sw * (yBound + v) + threadIdx.x + 2) + 0] = sour[channels * (width * (y + v) + x) + 0];
            if(threadIdx.x == blockDim.x - 1){
                #pragma unroll 2
                for(int i = 1; i < 3; i++){
                    if(bound_check(x+i, 0, width)){
                        s[channels * (sw * (yBound + v) + threadIdx.x + 2 + i) + 2] = sour[channels * (width * (y + v) + x + i) + 2];
                        s[channels * (sw * (yBound + v) + threadIdx.x + 2 + i) + 1] = sour[channels * (width * (y + v) + x + i) + 1];
                        s[channels * (sw * (yBound + v) + threadIdx.x + 2 + i) + 0] = sour[channels * (width * (y + v) + x + i) + 0];
                    }
                }
            }
        }
        ...
        for (int v = -yBound; v <= yBound; ++v) {
            ...
            for (int u = -xBound; u <= xBound; ++u) {
                if (bound_check(x + u, 0, width) && bound_check(y + v, 0, height)) {
                    const unsigned char R = s[channels * (sw * (yBound + v) + (threadIdx.x + 2 + u)) + 2];
                    const unsigned char G = s[channels * (sw * (yBound + v) + (threadIdx.x + 2 + u)) + 1];
                    const unsigned char B = s[channels * (sw * (yBound + v) + (threadIdx.x + 2 + u)) + 0];
...
int main(int argc, char **argv) {
    ...
    unsigned sizeShareMem = Y*(num_threads+4)*channels;
    // std::cout << num_blocks.x << ", " << num_blocks.y << ", " << num_blocks.z << std::endl;
    // launch cuda kernel
    sobel <<<num_blocks, num_threads, sizeShareMem>>> (dsrc, ddst, height, width, channels);
